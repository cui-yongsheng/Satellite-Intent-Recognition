function StationKeepingEW(satellite,lan,delta)
%EWSTATIONKEEPING 东西经度保持
%   satellite
%% 添加一个East-West位置保持的程序自动序列
% 定义AutoSequence，首先插入一个Sequence，重命名为EW_Station_Keeping。
satellite.Propagator.AutoSequence.Add('EW Station Keeping');
satellite.Propagator.AutoSequence.Item(1).Sequence.Insert('eVASegmentTypeSequence','EW_Station_Keeping','-');
EW=satellite.Propagator.AutoSequence.Item(1).Sequence.Item(0).Segment;
...
Min_lon.Enable=1;
Min_lon.DesiredValue=lan+delta;
