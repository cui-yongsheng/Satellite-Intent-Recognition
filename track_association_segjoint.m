...
%% 数据处理
view_angle = 60;
pos_obs = data_observation{5};
masks = cellfun(@(x) myseg(x, pos_obs, time_series, view_angle), data_propagator, 'UniformOutput', false);    % 提取卫星信息，去除时间信息
data_propagator = cellfun(@(x, mask) x .* (mask == 1), data_propagator, masks, 'UniformOutput', false);
...
%%
function visible_mask = myseg(x, pos_obs, time_series, view_angle)
    [~, ~, ~, visible_mask] = segmentVisible(x, pos_obs, time_series, view_angle);
end